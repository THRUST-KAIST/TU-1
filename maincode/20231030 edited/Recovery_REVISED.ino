...
void setup() {
  ...
  myFile.println("Time, mx, my, mz, ax, ay, az, gx, gy, gz");
  myFile.close();

  // myFile = SD.open("ptsensor.txt", FILE_WRITE);
  // SD.remove("ptsensor.txt");

  myFile = SD.open("ptsensor3.txt", FILE_WRITE);
...
void loop() {
  ...
  myFile = SD.open("rectest3.txt", FILE_WRITE);
  myptFile = SD.open("ptsensor3.txt", FILE_WRITE);
